 ...
 */
function quat_tran_setup()
{
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on quat-tran, use a find and replace
	 * to change 'quat-tran' to the name of your theme in all the template files.
	 */
	load_theme_textdomain('quat-tran', get_template_directory() . '/languages');
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'quat_tran_custom_background_args',
			array(
...
add_action('after_setup_theme', 'quat_tran_setup');
 ...
 */
function quat_tran_content_width()
{
	$GLOBALS['content_width'] = apply_filters('quat_tran_content_width', 640);
}
add_action('after_setup_theme', 'quat_tran_content_width', 0);
 ...
 */
function quat_tran_widgets_init()
{
	register_sidebar(
		array(
			'name' => esc_html__('Sidebar', 'quat-tran'),
			'id' => 'sidebar-1',
			'description' => esc_html__('Add widgets here.', 'quat-tran'),
...
add_action('widgets_init', 'quat_tran_widgets_init');

/**
 * Enqueue scripts and styles.
 */
function quat_tran_scripts()
{
	wp_enqueue_style('quat-tran-style', get_stylesheet_uri(), array(), _S_VERSION);

	// add vendor js
	wp_enqueue_script('quat-tran-script-vendor', get_template_directory_uri() . '/assets/js/vendor.js', array(), _S_VERSION, true);
